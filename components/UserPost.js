...
export default function UserPost({
    ...
    const [runtimeReactionCount, setTotalReactions] = useState(totalReactions); 

    const handleLike = async () => {
        try {
            setIsLiked((prevIsLiked) => !prevIsLiked); // Update isLiked immediately
            // Update the totalReactions count based on the like status
            setTotalReactions((prevTotalReactions) =>
                isLiked ? prevTotalReactions - 1 : prevTotalReactions + 1
            );
    ...
    const saveItemToLocalStorage = async (
        ...
        try {
            ...
            // Check if the item is already saved based on postID
            const isItemSaved = parsedSavedItems.some(
                (savedItem) =>
            ...
            if (!isItemSaved) {
                ...
                ToastAndroid.show("Item Saved! Check Notifications Tab.", ToastAndroid.LONG);
            } else {
                ToastAndroid.show("Item Already Saved! Check Notifications Tab.", ToastAndroid.LONG);
            }
        } catch (error) {
                    ...
                    <View style={styles.container}>
                        <TouchableOpacity onPress={handleLike}>
                            <View style={styles.likeContainer}>
                                ...
                                {isLiked ? (
                                    <Text style={styles.likeText}>{runtimeReactionCount}</Text>
                                ) : (
                                    <Text style={styles.likeText}>{runtimeReactionCount}</Text>
                                )}
                            </View>
